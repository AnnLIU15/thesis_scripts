...
class IMAP_Downemail(object):
    ...
    """
    def __init__(self, account, pwd, serverurl, savedir, startdate, enddate,
                 all_name, keywords, last_friday_date,
                 exts=['.xls', '.xlsx']):
        """
        init
        :param account:   邮箱账户
        ...
        if (not has_attachment) and len(attachments) == 0:
            for part in msg.walk():
                if part.get_content_maintype() == 'text':
                    ...
                    self.set_cell_font(table.rows[0].cells[0], 'Times New Roman', 16, '宋体')
                    table.rows[0].cells[1].text = name
                    self.set_cell_font(table.rows[0].cells[1], 'Times New Roman', 16, '宋体')
                ...
                try:
                    ...
                    if (mail_date > stopdate):
                        ...
                        if subject == '':
                            ...
                            for attachment,attachment_path in zip(attachments,attachments_path):
                                if process_fun:
                                    process_fun("\t已下载文件: {} -> {}".format(attachment,attachment_path))
                        else:
                            ...
                            if ('周报' in subject) and name_exist_in_list:
                                print(mail_date, mail['From'],subject)
                                # 获取附件
                                attachments, attachments_path = self.__getEmailattachment(mail, subject)
                                for attachment,attachment_path in zip(attachments,attachments_path):
                                    if process_fun:
                                        process_fun("\t已下载文件: {} -> {}".format(attachment,attachment_path))
...
print(startdate, enddate, nearest_friday.strftime("%Y%m%d"),)
# 下载
for account in account_list:
    _email = account['email']
    _password = account['password']
    _server = account['server']
    etool = IMAP_Downemail(_email, _password, _server, output_folder,
                           startdate, enddate, all_name, keywords,nearest_friday.strftime("%Y%m%d"), exts=allowed_extensions)
...
for md_file_path in md_files:
    ...
    replace_str = f'第{chinese_number_dict[this_week]}周 ({six_days_ago_str}~{last_friday_date})'
    print(matches[0])
    if matches:
        ...
        markdown_content = re.sub(r'[‘’“”（）；。]', lambda x: chinese_to_english_mapping[x.group(0)], markdown_content)
        # 如果需要，你可以将修改后的Markdown内容保存回文件中
        with open(md_file_path, "w", encoding="utf-8") as file:
            file.write(markdown_content)
        # 新的MD文件路径
        new_md_file = (f'第{chinese_number_dict[this_week]}周周报_'
                        f'{name}_{last_friday_date}.md')
        new_md_file_path = (f'{file_dir}/'
                            f'{new_md_file}')
        ...
        convert_md_to_pdf_pypandoc(new_md_file_path, pdf_file_path, 'pandoc-template.latex')
