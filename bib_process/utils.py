    ...
    if not contains_chinese(entry["author"]):
        ...
        if is_comma:
            for idx, author in enumerate(multi_author):
                ...
                factor = factor_pre - (
                    ("{" == author[0]) and ("}" == author[last_name_pos - 1])
                )
                multi_author[idx] = (
                    "{" * factor
                    + author[:last_name_pos]
                    + "}" * factor
                    + author[last_name_pos:]
                )
        else:
            for idx, author in enumerate(multi_author):
                ...
                factor = factor_pre - (
                    ("{" == last_name_pos[-1][0]) and ("}" == last_name_pos[-1][-1])
                )
    ...
    try:
        ...
        for val in entry["title"].split(" "):
            new_name_list.append(
                "{" * factor_pre + strip_outer_braces(val) + "}" * factor_pre
            )
...
#   title    = {abbrevIso},
#   url      = {https://github.com/marcinwrochna/abbrevIso},
# }
# Thanks to https://github.com/marcinwrochna/abbrevIso?tab=readme-ov-file
    ...
    for url in urls:
        try:
            ...
            if response.status_code == 200:
                return (
                    response.text
                )  # Use `.text` instead of `.content.decode()` for simplicity
        except requests.RequestException as e:
            print(f"Error accessing {url}: {str(e)}")  # Better error handling

    raise ValueError(
        "Error: Unable to retrieve abbreviation\n"
